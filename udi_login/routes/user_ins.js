...
//========================================================================
//  curl -X POST -H "Content-Type: application/json" -d "{\"userid\":\"user7000\", \"username\":\"내이름\"}" http://127.0.0.1:13890/user/ins
{
  ...
  try {
    ret_data = await sqlconn.query("CALL pc_tuser_ins(?,?,@res, @res2)", [req_userid, req_username]); 
    //console.log(LOG_HEADER+"CALL pc_tuser_ins("+req_userid+", "+req_username+")=>", ret_data);
    ...
    ret_data = {
      code: "CALL pc_tuser_ins(userid:"+req_userid+", username:"+req_username+")",
